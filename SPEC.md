...
- 147 篇公開文章，直接顯示全文。
- 58 篇草稿或隱藏文章，列表只顯示標題，文章頁使用閱讀確認遮罩。
- 452 則公開留言；非公開文章下的公開留言隨文章正文一起展開。

### 6.2 非公開資料

23 則原本標記為隱藏的留言必須解析並保存於標準化資料中，但不得輸出至本機或正式網站。

閱讀確認遮罩的共用密碼固定為 `111111`。這項功能只用來避免訪客直接閱讀，不是安全加密、登入或權限控制。文章與公開留言仍存在於正式網站的靜態 HTML，能檢視網頁原始碼的訪客可以讀取。
...
- 每頁 10 篇。
- 公開文章顯示日期、標題、摘要與留言數。
- 隱藏及草稿文章只顯示日期、標題與狀態標籤，鎖頭符號顯示於文章標題右側；不顯示摘要、標籤及留言數。
- 隱藏及草稿文章標題下方顯示「點擊標題輸入密碼解鎖」。
- 分頁導覽。
...
- 標題下方顯示「閱讀確認」。
- 密碼輸入框的提示文字為「輸入密碼」。
- 「展開文章」按鈕與 Enter 鍵皆可送出。
- 密碼錯誤時顯示提示，正文與留言維持隱藏。
- 密碼 `111111` 正確時展開正文、標籤、文章編號與公開留言。
- 重新整理頁面後恢復遮罩。
...
`include_drafts` 的網站層級預設為 `false`，本機及正式環境覆寫為 `true`。`include_hidden_comments` 維持 `false`。
...
- `dist/production/` 顯示全部 205 篇文章；隱藏與草稿文章使用閱讀確認遮罩。此目錄必須加入 Git。
...
```json
{
  ...
  "comments_public": 452,
  "comments_hidden": 23,
  "comments_on_generated_posts": 452,
  "comments_generated": 452,
...
- 本機與正式環境皆產生 205 個文章頁面。
- 顯示 452 則公開留言，23 則隱藏留言不輸出。
- 草稿與隱藏文章出現在導覽及封存中，但列表不得顯示正文摘要。
- 草稿與隱藏文章頁面必須通過閱讀確認後才在畫面上展開正文與留言。
...
1. 可重複執行的備份解析與網站建置程式。
2. 包含全部文章及留言的標準化保存資料。
3. 公開文章直接顯示，草稿與隱藏文章使用閱讀確認遮罩的靜態 Blog。
